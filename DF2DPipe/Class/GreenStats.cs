    ...
    class GreenStats : EconomyStats
    {
        ...
        public override Dictionary<string, List<IFeature>> GetStatsResult()
        {
            try
            {
                ...
                while ((disFeature = disCursor.NextFeature()) != null)
                {
                    ...
                    int greenCount = _fc.FeatureCount(filter);
                    ...
                    {
                        area += Convert.ToDouble(fcFeature.get_Value(IndexOfFcArea));
                        _listF.Add(fcFeature);
                    }
                    double fraction = Math.Round(area / disArea, 2) * 100;
                    _dict[disName] = _listF;
                    DataRow row = _dt.NewRow();
                    row["DISTRICTNAME"] = disName;
                    row["DISTRICTAREA"] = Math.Round(disArea, 2);
                    row["NUMBER"] = greenCount;
                    row["AREA"] = Math.Round(area, 2).ToString();
                    row["FLOORAREA"] = fraction.ToString() + "%";
                    _dt.Rows.Add(row);
                }
            }
        ...
        public override void InitDataTable()
        {
            ...
            _dt.Columns.AddRange(new DataColumn[]{new DataColumn("DISTRICTNAME"),
                new DataColumn("DISTRICTAREA",typeof(double)),new DataColumn("NUMBER",typeof(int)),new DataColumn("AREA"),new DataColumn("FLOORAREA")});
        }
