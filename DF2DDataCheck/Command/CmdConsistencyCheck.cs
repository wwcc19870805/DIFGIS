...
namespace DF2DDataCheck.Command
{
    class CmdConsistencyCheck : AbstractMap2DCommand
    {
        ...
        public override void Run(object sender, EventArgs e)
        {
            ...
            try
            {
                List<DF2DFeatureClass> pPntlist = Dictionary2DTable.Instance.GetFeatureClassByFacilityClassName("PipeNode");
                List<DF2DFeatureClass> pArclist = Dictionary2DTable.Instance.GetFeatureClassByFacilityClassName("PipeLine");
                if ((pPntlist == null) || (pArclist == null)) return;
                Dictionary<IFeatureClass, DataTable> dict = new Dictionary<IFeatureClass, DataTable>();
                WaitForm.Start("开始数据一致性检查..", "请稍后");
                foreach (DF2DFeatureClass Pntdfcc in pPntlist)
                {
                    DF2DFeatureClass Arcdfcc = pArclist[i];
                    ...
                    DFDataConfig.Class.FieldInfo fi = fac.GetFieldInfoBySystemName("Character");
                    if (fi == null) continue;
                    ...
                    DFDataConfig.Class.FieldInfo fi4 = fac.GetFieldInfoBySystemName("UseState");
