/*----------------------------------------------------------------------------------------
			...
			// 文件功能描述：给定圆心＋半径＋起角＋止角，绘制弧\扇形区域
			//
			// 
			// 创建标识：YuanHY 20051226
            // 操作说明：按shift键，修改给定圆心＋半径＋起角＋止角
            //           P键平行尺
			//　　　　　 ESC键取消所有操作
...
namespace WSGRI.DigitalFactory.DFEditorTool
{
    ...
    public class DrawArcCenRadFATA:AbstractMapCommand
    {
		...
		private string	strCaption  = "给定圆心 + 半径 + 起角 + 止角，绘制弧/扇形区域" ;
	    ...
	    public override void Execute()
        {
			...
			m_pStatusBarService.SetStateMessage("提示：依次指定1.圆弧中心;2.起始角(半径);3.终止角。(P:平行尺/ESC:取消/ENTER:结束/+shift:修改坐标)");
        ...
        {
            // TODO:  添加 DrawArcCenRadFATA.UnExecute 实现
			m_pStatusBarService.SetStateMessage("就绪");
        }

        public override void OnMouseDown(int button, int shift, int x, int y, double mapX, double mapY)
        {
            // TODO:  添加 DrawArcCenRadFATA.OnMouseDown 实现
            base.OnMouseDown (button, shift, x, y, mapX, mapY);

			m_pStatusBarService.SetStateMessage("依次指定:1.圆弧中心;2.起始角(半径);3.终止角。(P:平行尺/ESC:取消/ENTER:结束/+shift:修改坐标)");
        ...
        {
            // TODO:  添加 DrawArcCenRadFATA.OnBeforeScreenDraw 实现
