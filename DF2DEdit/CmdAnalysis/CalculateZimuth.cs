...
{
	/// <summary>
	/// CalculateZimuth 的摘要说明。
	/// </summary>
	public class CalculateZimuth:AbstractMapCommand
	{
		...
		private string	strCaption  = "计算方位角";
		private string	strCategory = "工具";


		public CalculateZimuth()
		{
			//获得状态栏的服务
...
//		}
		...
		{
			// TODO:  添加 CalculateZimuth.UnExecute 实现
 ...
 　　　　       m_pPoint1 = pPoint;
			...
			else //如果命令正在使用
			{
				...
				string  strResult = "方位角:" + dblZimuth.ToString(".#####") + " (°′″) ";
				        strResult = strResult + CRLF;
				        strResult = strResult + "第一点坐标 X=" + m_pPoint1.X.ToString(".###")  + "Y=" + m_pPoint1.Y.ToString(".###"); 
						strResult = strResult + CRLF;
						strResult = strResult + "第二点坐标 X=" + m_pPoint2.X.ToString(".###")  + "Y=" + m_pPoint2.Y.ToString(".###"); 
            
				result = MessageBox.Show(strResult, "方位角计算",	MessageBoxButtons.OK,MessageBoxIcon.Information);
		...
		{
			// TODO:  添加 CalculateZimuth.OnBeforeScreenDraw 实现
